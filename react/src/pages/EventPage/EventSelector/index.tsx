...
const RoomSelector = ({
  ...
  const { data: rooms } = useQuery(["room", propertyId], () =>
    getRooms(propertyId)
  );
  ...
  const handleSelection = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSelectedRoomNumbers((prev) =>
      checked
        ? [...prev, roomNumberId]
        : prev.filter((item) => item !== roomNumberId)
    );
    ...
    rooms?.forEach((room) => {
      room.roomNumbers.forEach((roomNumber) => {
        if (roomNumber._id === roomNumberId) {
          setTotalAmount((prev) =>
            checked ? prev + room.price : prev - room.price
          );
          return;
        }
      });
        ...
        if (selectedRoomNumbers.includes(roomNumber._id)) {
          ...
          if (objIndex !== -1) {
            ...
            _rooms.push({
              id: room._id,
              roomNumbers: [
                { number: roomNumber.number, unavailableDates: dates },
              ],
            });
          }
        }
